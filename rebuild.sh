...
docker rm -f buildkit && 'echo Deleted' || echo 'Nothing to delete'
echo "[Making folders for the mounted volumes]"
mkdir -p ampuser build
echo "[Making a container based on the image]"
docker create  -p 2222:2222  -p 8001:8001  -p 8000:80  -v $(pwd)/ampuser:/var/www -v $(pwd)/build:/opt/buildkit/build --name buildkit dcbk:v1
...
You can open a bash shell with this command:
    ...
    docker exec -it buildkit su -l www-data
